...
- **输入：** prompt $q$
- **输出：** 自回归生成的轨迹 $T = (a_1, \dots, a_{|T|})$，其中 $\pi(a_t \mid q, a_{<t})$ 为每步 token 的条件分布
...
$$\hat{H}_{\text{tok}}(\pi) = \frac{1}{N} \sum_{i=1}^{N} \sum_{t=1}^{|T^i|} H\bigl(\pi(\cdot \mid a^i_{<t})\bigr)$$
...
$$r_{\text{traj}}(T) = \sum_{t=1}^{|T|} \log \pi(a_t \mid a_{<t}) = \log \pi(T)$$

（论文符号：$r_{\text{traj}}(y) = \log \pi_\theta(y)$）

**EM-RL-token**（token 级熵）：

$$r_{\text{tok}}(T) = -\sum_{t=1}^{|T|} H\bigl(\pi(\cdot \mid a_{<t})\bigr)$$
...
**核心思路：** 不更新模型参数 $\theta$，而是将每步的 logit 向量 $z_t \in \mathbb{R}^{|V|}$ 视为可优化的自由参数，通过梯度下降最小化其诱导分布的熵。
